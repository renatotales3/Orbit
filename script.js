document.addEventListener('DOMContentLoaded', () => {
    ...
    allNavButtons.forEach(button => button.addEventListener('click', () => {
    ...
    const getPomodoroTimes = () => { return loadFromLocalStorage('pomodoroTimes', { focus: 25, shortBreak: 5, longBreak: 15 }); };
    const savePomodoroTimes = () => { const times = { focus: parseInt(focusTimeInput.value) || 25, shortBreak: parseInt(shortBreakTimeInput.value) || 5, longBreak: parseInt(longBreakTimeInput.value) || 15 }; saveToLocalStorage('pomodoroTimes', times); setTimerForCurrentCycle(); };
    const loadPomodoroSettings = () => { const times = getPomodoroTimes(); focusTimeInput.value = times.focus; shortBreakTimeInput.value = times.shortBreak; longBreakTimeInput.value = times.longBreak; };
    focusTimeInput.addEventListener('change', savePomodoroTimes);
    shortBreakTimeInput.addEventListener('change', savePomodoroTimes);
    longBreakTimeInput.addEventListener('change', savePomodoroTimes);
    ...
    const formatDateToBR = (dateString) => { if (!dateString) return ''; const [year, month, day] = dateString.split('-'); return `${day}/${month}/${year}`; };
