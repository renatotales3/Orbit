document.addEventListener('DOMContentLoaded', () => {
        ...
        const createTaskHTML = task => {
            ...
            return `<li class="task-item ${task.completed ? 'completed' : ''}" data-id="${task.id}" ${dataAttrs}>
                        <div class="task-item-content">
                            ...
                            <span>${Utils.escapeHTML(task.text)}</span>
                        </div>
    ...
    const Journal = (() => {
        ...
        const createHistoryItemHTML = (entry) => ` <li class="journal-history-item"> <p class="history-date">${Utils.formatDateToBR(entry.date)}</p> <p class="history-question">${Utils.escapeHTML(entry.journal.question)}</p> <p class="history-answer">${Utils.escapeHTML(entry.journal.answer)}</p> </li> `;
