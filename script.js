document.addEventListener('DOMContentLoaded', () => {
    ...
    const Tasks = (() => {
        ...
        const createTaskHTML = task => {
            ...
            return `<li class="task-item ${task.completed ? 'completed' : ''}" data-id="${task.id}" ${dataAttrs} draggable="true">
        ...
        const init = () => {
            ...
            // Swipe Gestures para Tarefas
            let startX = 0, currentX = 0, isDragging = false, currentTaskItem = null;
            ...
            taskList.addEventListener('touchmove', (e) => {
                if (isDragging && currentTaskItem) {
                    ...
                    currentTaskItem.style.transform = `translateX(${clampedDiff}px)`;
                    currentTaskItem.classList.remove('swiping-left', 'swiping-right');
                    
                    if (Math.abs(clampedDiff) > 40) {
                        currentTaskItem.classList.add(clampedDiff > 0 ? 'swiping-right' : 'swiping-left');
                    }
                }
            }, { passive: false });
            ...
            taskList.addEventListener('touchend', () => {
                if (isDragging && currentTaskItem) {
                    ...
                    currentTaskItem.style.transition = 'transform 0.3s ease, opacity 0.3s ease';
                    
                    if (Math.abs(diffX) > threshold) {
                        const taskId = parseInt(currentTaskItem.dataset.id);
                        const taskIndex = tasks.findIndex(t => t.id === taskId);
                        
                        if (taskIndex > -1) {
                            if (diffX > 0) {
                                // Swipe right - completar/descompletar
                                currentTaskItem.classList.add('completing');
                                setTimeout(() => {
                                    tasks[taskIndex].completed = !tasks[taskIndex].completed;
                                    
                                    // Integração com Goals se for uma subtask
                                    const { goalId, subtaskId } = tasks[taskIndex];
                                    if (goalId && subtaskId) {
                                        Goals.setSubtaskCompletedState(Number(goalId), Number(subtaskId), tasks[taskIndex].completed);
                                    }
                                ...
                                // Swipe left - excluir
                                currentTaskItem.classList.add('deleting');
                                setTimeout(() => {
                                    tasks.splice(taskIndex, 1);
                                    Utils.saveToLocalStorage('tasks', tasks);
                                    render();
                        ...
                        currentTaskItem.style.transform = 'translateX(0)';
                        currentTaskItem.classList.remove('swiping-left', 'swiping-right');
                    }
            ...
            taskList.addEventListener('dragover', (e) => {
                ...
                if (afterElement == null) {
                    ...
                    if (lastItem && lastItem !== draggedElement) {
                        lastItem.classList.add('drag-over');
                    }
